...
def main():
    ...
    parser.add_argument("--csv_path", required=True, help="Path of the input CSV file")
    parser.add_argument(
        "--prompts_file",
        default=None,
    ...
    parser.add_argument("--device", default="cuda:1", help="The GPU in use")
    parser.add_argument("--log_file", default="model_eval.log")
    parser.add_argument(
        "--force-overwrite",
        action="store_true",
    ...
    parser.add_argument(
        ...
        default="utf-8",
        help=(
            "Encoding used to read the input CSV and existing checkpoints, and to write results. "
        ),
    )
    ...
    logger.info(f"Mapping key: {args.mapping_key or csv_stem}")
    ...
    for prompt in prompt_names:
        ...
        if prompt_requires_image(prompt_content) and not model_instance.supports_image:
            logger.info(
                f"Skip '{prompt}' on {args.model_name}: "
        ...
        if Path(out_csv).exists() and not args.force_overwrite:
            logger.info(f"Resuming from existing checkpoint: {out_csv}")
        else:
            if args.force_overwrite and Path(out_csv).exists():
                logger.info(f"Force overwrite enabled, removing existing file: {out_csv}")
                Path(out_csv).unlink()
            logger.info(f"Starting new results file: {out_csv}")
        ...
        logger.info(f"Completed processing: {out_csv}")
